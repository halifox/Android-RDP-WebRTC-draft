...
class ScreenCaptureServiceNettyImage : ScreenCaptureService0() {
    ...
    var count = 0
    var lastSendTime = 0L
    fun onImageAvailable0(image: Image) {
        ...
        val msg = byteArrayOutputStream.toByteArray()


        Log.d("TAG", "writeAndFlush: ")
        if (System.currentTimeMillis() > lastSendTime + 100) {
            Log.d("TAG", "writeAndFlush:send[${count}] ${msg.size}")
            if (ctx != null) {
                count++
            }
    ...
    private val imageReader = ImageReader.newInstance(ScreenUtils.getScreenWidth(), ScreenUtils.getScreenHeight(), PixelFormat.RGBA_8888, 2)
        .apply {
            setOnImageAvailableListener(::onImageAvailable, android.os.Handler(handlerThread.looper))
        }
